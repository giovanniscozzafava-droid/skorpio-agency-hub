...
export function DailyPriorityManager({ team, utente }: Props) {
  ...
  const addPriority = async (taskId: string, persona: string) => {
    setSaving(true);
  ...
  const availableTasks = tasks.filter(t => {
    if (t.assegnato_a !== selectedPerson) return false;
    if (priorities.find(p => p.task_id === t.id && p.assegnato_a === selectedPerson && p.attivo)) return false;
    if (searchQuery && !(t.descrizione + t.cliente_nome + t.id_display).toLowerCase().includes(searchQuery.toLowerCase())) return false;
    return true;
  });
            ...
            <div className="max-h-[50vh] overflow-y-auto p-2">
              {availableTasks.length === 0 ? (
                ...
                availableTasks.slice(0, 20).map(t => (
                  <button key={t.id} onClick={() => addPriority(t.id, selectedPerson)} disabled={saving}
